...
1. **Parsed definitions**: the native preflight parses every project-owned stylesheet retained from the entry import graph and returns all authored `@custom-variant` names with normalized definitions, while existing theme breakpoint names remain reserved through the theme-token graph.
2. **The loaded design system**: before committing to any name, the orchestration layer compiles a probe candidate against the unaugmented design system, which also reveals variants registered by an `@plugin` or defined in dependency-owned imported stylesheets. Custom properties follow the same ownership-blind rule, because a generated `--breakpoint-*` variable is a plain custom property once emitted: a candidate name that any loaded stylesheet already declares, inside or outside a `@theme` block and whether project-owned or dependency-owned, is reserved so no existing property value is overwritten.
3. **The scanned candidate corpus**: the probe cannot see inert candidates such as `screen-width-lte-768px:hidden` sitting in a file inside the entry's detection scope, which a generated definition would activate. The preflight therefore scans the entry's provable detection scope for candidate tokens, parses each candidate's variant chain recursively, and reserves every referenced variant and leaf breakpoint name rather than only literal prefixes. An inert `max-min-52rem:hidden` therefore reserves `min-52rem`. When the detection scope cannot be enumerated, generated names for the entry are not allocated and its conditions fall back to arbitrary variants.

Probing is bounded rather than open-ended: when the readable name and its digest-suffixed form both resolve, the readable prefix is treated as owned by an existing functional variant namespace that suffixing cannot escape, and the allocator probes one name in the reserved generated namespace `twm-media-<digest>` instead. A condition whose namespaced name also resolves falls back to its arbitrary variant rather than failing the entry group.

Reuse is stricter than reservation. The allocator reuses an existing name only when its effective behavior is proven: the name has exactly one authored project-owned definition, that definition's normalized meaning matches the condition, no competing registration from another stylesheet or plugin exists, and a compiled probe expands to the authored condition. A duplicate or opaque registration counts as a collision, and a name known only through the probe is never reused.
...
6. emit generated custom variants in the definition order defined below;
...
- Order generated custom variants by proven cascade position and extend conflict analysis to overlapping generated conditions of both definition kinds.
...
- an authored or dependency-owned `--breakpoint-*` custom property outside `@theme` reserves its name and forces the suffixed generated form;
- a variant name with competing registrations counts as a collision and is never reused;
- ordering-sensitive pairs involving reused custom-media variants follow the same retention rules as generated definitions;
- same-stylesheet overlapping generated conditions preserve the original winning declaration through definition order;
...
- variants and recursively referenced leaf names found in the entry's scanned candidate corpus are reserved before allocation;
...
7. Automatic shared-entry coverage is intentionally narrow: ancestry supplies candidates, a direct reference supplies loading proof, and a provable source scope supplies scan proof, with both proofs required.
