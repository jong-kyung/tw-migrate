...
Supported `@media`, `@supports`, `@container`, and `@starting-style` blocks are traversed recursively, including conditions nested inside style rules. Exact built-ins become stacked variants such as `motion-reduce:starting:@md:grid`; compound media and named or ranged container queries use equivalent Tailwind arbitrary at-rule variants. If a condition or nested statement cannot be represented safely, its outer block and related CSS Module classes remain unchanged.

Local CSS Module `@keyframes` referenced by a single static `animation` or `animation-name` are renamed deterministically and moved to the Tailwind entry before the module is removed. Global definition at-rules such as `@font-face`, `@page`, `@property`, `@counter-style`, and `@view-transition` are also moved before removal. Relative URLs move only when the CSS Module and Tailwind entry share a directory; absolute and data URLs remain stable. Multiple animations, ambiguous names, path-changing relative URLs, dynamic values, and structural at-rules remain unchanged.
