...
The override scan proves absence only inside the migration snapshot. A consumer context may keep a style source the snapshot cannot inspect, such as a remote stylesheet link, an untouched inline style block, or a script that produces styles at runtime, and that source can set the same custom property invisibly. Any such opaque style source in a context that loads the entry therefore reserves every theme-backed alias it could affect, and the affected candidates keep their arbitrary spellings.
...
3. Reserve names whose `font-{name}` spelling appears as a class selector in any stylesheet snapshot, including stylesheets outside the entry graph and scan corpus, because migrated elements carrying the generated utility would match those authored rules.
