...
export async function withTimeout<T>(operation: () => T | Promise<T>, timeoutMs = captureAttemptTimeoutMs): Promise<T> {
...
async function waitForCardinality(target: Locator, expected: number, timeoutMs: number): Promise<void> {
...
async function capturePage(
  ...
  try {
    ...
    if (failure && artifact?.screenshot && failedPage) {
      await withTimeout(() => failedPage.screenshot({ path: artifact.screenshot, fullPage: true }), remaining()).catch(() => {});
    }
...
export async function captureAll(
  ...
  const captures = await Promise.allSettled(Object.entries(probes).map(async ([name, probe]): Promise<[string, ProbeCapture]> => [
    name,
  ...
  return Object.fromEntries(captures.flatMap((capture) => capture.status === 'fulfilled' ? [capture.value] : []));
}
    ...
    capture.elements.forEach((element, index) => {
      ...
      for (const [property, value] of Object.entries(element.styles)) {
        if (!property.startsWith('--') && absent?.styles[property] !== value) {
          probeWitnesses.push({ identity: element.identity, property, baseline: value, withheld: absent?.styles[property] });
        }
      }
    });
    assert.ok(
