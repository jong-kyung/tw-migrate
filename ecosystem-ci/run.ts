...
function validateExpectation(value: unknown, label: string): void {
  const expectation = exactKeys(value, ['selector', 'cardinality'], ['selector', 'cardinality'], label);
...
function validateProbe(value: unknown, label: string): void {
  ...
  probe.identity.forEach((identity: unknown, index: number) => nonempty(identity, `${label}.identity[${index}]`));
...
function validateProbes(value: unknown, label: string, controlled: boolean): void {
  ...
  const probe = (name: string) => probes[name] as { action?: { type?: string; key?: string }; viewport: { width: number } };
  for (const [name, type] of [['hover', 'hover'], ['focus', 'focus'], ['focus-visible', 'press']] as const) {
...
function validateCommand(command: unknown, label: string): void {
  if (!Array.isArray(command) || command.length === 0 || command.some((part: unknown) => typeof part !== 'string' || part.length === 0)) {
...
function validateExternalInstall(manager: string, value: unknown, label: string): void {
  ...
  const exact = (expected: string[]) => args.length === expected.length && args.every((part, index) => part === expected[index]);
...
function validateProject(value: unknown, index: number): void {
  ...
  if (project.kind === 'controlled') {
    ...
    if (!runtimes.has(project.runtime as string)) throw new Error(`${label}.runtime is unsupported`);
    ...
    let repository: URL;
    try { repository = new URL(project.repository as string); } catch { throw new Error(`${label}.repository must be an HTTPS URL`); }
    ...
    if (!/^[0-9a-f]{40}$/.test(String(project.revision))) throw new Error(`${label}.revision must be a full 40-character SHA`);
    ...
    project.runtimeWrites.forEach((path: unknown, pathIndex: number) => validateRelativePath(path, `${label}.runtimeWrites[${pathIndex}]`));
    ...
    if (!['vite', 'next'].includes(project.server as string)) throw new Error(`${label}.server must be vite or next`);
  } else {
  ...
  if (project.kind === 'external') {
    ...
    if ((project.runtimeWrites as string[]).some((path) => protectedPaths.has(posix.normalize(path)))) {
...
export async function loadManifest(url: URL = new URL('./projects.json', import.meta.url)): Promise<Manifest> {
  return validateManifest(JSON.parse(await readFile(url, 'utf8')));
}

export function vitestProjects(projects: Project[], env: NodeJS.ProcessEnv = process.env): Project[] {
...
export function runHarness(args: string[], manifest: Manifest, execute: (args: string[]) => void = executeVitest): Project[] {
  validateManifest(manifest);
  const selected = selectProjects(args, manifest);
  execute(['run', '--config', 'ecosystem-ci/vitest.config.ts', ...selected.flatMap(({ id }) => ['--project', id])]);
  return selected;
}
